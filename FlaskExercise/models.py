	...
	def save_changes(self, file):
		if file:
			...
			try:
				...
				blob_client_new = blob_service.get_blob_client(container=blob_container,blob=filename)
				blob_client_new.upload_blob(file)
				pass
				if self.image_path:
					# TODO: Get a blob client and delete the previous blob
					blob_client_old = blob_service.get_blob_client(container=blob_container,blob=self.image_path)
